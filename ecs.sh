...
ipcheck() {
    if [ "$en_status" = true ]; then
        ...
        _blue "ipapiis databases ⑪ | ipapicom databases    ⑫  | abstractapi databases ⑬ | ipqualityscore databases ⑭ "
    else
    ...
    if [[ -n "$score_2_4" && -n "$score_14_4" ]]; then
        if [ "$en_status" = true ]; then
            temp_text+="Fraud_score(the lower the better): $score_2_4⑤  $score_14_4⑪  "
        else
            temp_text+="欺诈分数(越低越好): $score_2_4⑤  $score_14_4⑪  "
        fi
    elif [[ -n "$score_2_4" && -z "$score_14_4" ]]; then
        if [ "$en_status" = true ]; then
            temp_text+="Fraud_score(the lower the better): $score_2_4⑤  "
        else
